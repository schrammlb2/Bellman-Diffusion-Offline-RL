...
class SSMBCPolicy(ReBRACPolicy):
    """
    ...
    def __init__(
        ...
        on_policy_diffusion_model: nn.Module,
        ...
        self.on_policy_diffusion_model = on_policy_diffusion_model
        self.on_policy_diffusion_model_old = deepcopy(on_policy_diffusion_model)
        self.on_policy_diffusion_model_optim = on_policy_diffusion_model_optim
    ...
    def state_bc(self, obss, a=None):
        ...
        on_policy_obs_prediction = self.on_policy_diffusion_model_old(
            x=noised_pred_obss, obs=obss, 
            step=self.map_i(diff_steps),
            actions=a
        )
        state_bc_penalty = self.relative_state_reg_weight*self.actor_action_reg_weight*(
            eta*((on_policy_obs_prediction - obs_prediction)**2).sum(-1)
        ).mean()
