...
class VarianceReducedSOMPolicy(TD3Policy):
    """
    ...
    def predict(self, noise, obs, a):
        ...
        x = noise
        x_list = []
        diff_steps = torch.ones((x.shape[0],1))#, device=self.device)
    ...
    def learn(self, batch: Dict) -> Dict[str, float]:
        ...
        if self._cnt % self._freq == 0:
            ...
            for _ in range(self.num_diffusion_iters):
                ...
                # shuffled_obs_data_prediction = self.data_diffusion_model_old(
                #     x = noised_shuffled_obss, step=map_i(diff_steps),
                # )
                # rkl_list.append((shuffled_obs_prediction - shuffled_obs_data_prediction)**2)
        ...
        def estimate_diffusion_rkl_grad():
            dist = self.actor(obss)
            atanh_a = dist.rsample()[1]
            tanh_a = torch.tanh(atanh_a)
            ...
            # rkl_list.append((shuffled_obs_prediction - shuffled_obss)**2)
            rkl_list.append((shuffled_obs_prediction - shuffled_obs_data_prediction)**2)
            loss = ((shuffled_obs_prediction - shuffled_obs_data_prediction)**2).mean()
        ...
        def estimate_rkl_weight_grad():
            ...
            grad = torch.cat(grad_list).unsqueeze(0)
            self.actor_optim.zero_grad()
            return grad


        def estimate_covariance(fn1, fn2=None, label=""):
            rkl_list_1 = []
            rkl_list_2 = []
            if fn2 == None: 
                fn2 = fn1
            for _ in range(self.num_diffusion_iters):
                rkl_list_1.append(fn1())
                rkl_list_2.append(fn2())

            rkl_tensor_1 = torch.stack(rkl_list_1, -1)
            rkl_tensor_2 = torch.stack(rkl_list_2, -1)

            assert len(rkl_tensor_1.shape) == 3
            assert rkl_tensor_1.shape[0] in [1, obss.shape[0]]
            assert rkl_tensor_1.shape[2] == self.num_diffusion_iters

            cos = torch.nn.functional.cosine_similarity
            cos_sim_torch = cos(rkl_tensor_1, rkl_tensor_2, dim=-2).mean()
            cos_sim_means_torch = cos(rkl_tensor_1.mean(-1), rkl_tensor_2.mean(-1), dim=-1).mean(0)

            return {
                f"metric/{label}_cos": cos_sim_torch.item(), 
                f"metric/{label}_m_cos": cos_sim_means_torch.item()
            }

        if self._cnt%250==0: 
            self._results_dict = {}
            self._results_dict.update(estimate_covariance(estimate_rkl_grad, label="a"))
            self._results_dict.update(estimate_covariance(estimate_diffusion_rkl_grad, label="diff_a"))
            self._results_dict.update(estimate_covariance(estimate_rkl_grad, fn2=estimate_diffusion_rkl_grad, 
                label="diff_cov"))
            # self._results_dict.update(estimate_covariance(estimate_rkl_weight_grad, "rkl_w"))
            # self._results_dict.update(estimate_variance(estimate_full_weight_grad, "full_w"))
        ...
        return_dict = {
            ...
            "loss/uncond_diffusion": data_diffusion_loss.item()
        }
        return_dict.update(self._results_dict)
        return return_dict