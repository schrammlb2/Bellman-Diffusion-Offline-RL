...
class SOMRegOnlyPolicy(TD3Policy):
    """
    ...
    def learn(self, batch: Dict) -> Dict[str, float]:
        ...
        if self._cnt % self._freq == 0:
            ...
            actor_loss = -0*lmbda * q.mean() + (
                1*(((atanh_a - atanh_actions).pow(2)*sigma**(-2)).mean() + 2*torch.log(sigma).mean())
                + 1*((shuffled_obs_prediction - shuffled_obss)**2).mean()
            )
