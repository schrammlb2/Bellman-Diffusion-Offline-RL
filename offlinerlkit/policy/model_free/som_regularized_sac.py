...
class SOMRegularizedSACPolicy(TD3Policy):
    ...
    def learn(self, batch: Dict) -> Dict[str, float]:
        ...
        if self._cnt % self._freq == 0:
            ...
            action_kl = (((atanh_a - atanh_actions).pow(2)*sigma**(-2)).mean() + 2*torch.log(sigma).mean())
            state_kl = self.num_diffusion_iters*((shuffled_obs_prediction - shuffled_obss)**2).mean()
            # actor_loss = -2*lmbda * q.mean() + action_kl + state_kl
            # actor_loss = -5*lmbda * q.mean() + torch.sqrt((action_kl + state_kl))
            actor_loss = -2*lmbda * q.mean() + (action_kl + 1*state_kl)
