...
class DiffusionPolicy(TD3Policy):
    """
    ...
    def policy_predict(self, model, noise, obs):
        x = noise
        x_list = []
        diff_steps = torch.ones((obs.shape[0],1))#, device=self.device)
        for i in range(self.num_diffusion_iters-1, 0, -1):
    ...
    def update_data_diffusion(self, batch):  
        obss, actions, next_obss, rewards, terminals = batch["observations"], batch["actions"], \
            batch["next_observations"], batch["rewards"], batch["terminals"]
        ...
        return data_diffusion_loss


    def update_critic(self, batch, pred_next_actions):
        obss, actions, next_obss, rewards, terminals = batch["observations"], batch["actions"], \
            batch["next_observations"], batch["rewards"], batch["terminals"]
        valid_next_actions = batch["valid_next_actions"]
        next_actions = batch["next_actions"]
    ...
    def update_actor_loss(self, batch):
        obss, actions, next_obss, rewards, terminals = batch["observations"], batch["actions"], \
            batch["next_observations"], batch["rewards"], batch["terminals"]
        valid_next_actions = batch["valid_next_actions"]
        next_actions = batch["next_actions"]   
        ...
        temp_atanh_a_new = dist.rsample()[1]
        temp_tanh_a_new = torch.tanh(temp_atanh_a_new)
        temp_noise = torch.randn(obss.shape, device=obss.device)
        off_policy_obs = self.predict(
            model=self.diffusion_model_old,
            noise=temp_noise, obs=obss, a=temp_tanh_a_new
        ).detach()
        for obss in [obss, off_policy_obs]:
    ...
    def learn(self, batch: Dict) -> Dict[str, float]:
        ...
        critic1_loss, critic2_loss = self.update_critic(batch, pred_next_actions)
        diffusion_loss = self.update_diffusion(batch, pred_next_actions)
        data_diffusion_loss = self.update_data_diffusion(batch)

        # update actor
        if self._cnt % self._freq == 0:
            self.update_actor_loss(batch)
