...
class BehaviorRegSACPolicy(TD3Policy):
    """
    ...
    def __init__(
        ...
        bc_diffusion_model: nn.Module,
        actor_optim: torch.optim.Optimizer,
        critic1_optim: torch.optim.Optimizer,
        critic2_optim: torch.optim.Optimizer,
        diffusion_model_optim: torch.optim.Optimizer,
        bc_diffusion_model_optim: torch.optim.Optimizer,
        tau: float = 0.005,
        gamma: float  = 0.99,
        ...
        self.bc_diffusion_model = bc_diffusion_model
        self.bc_diffusion_model_old = deepcopy(bc_diffusion_model)
        self.bc_diffusion_model_optim = bc_diffusion_model_optim
    ...
    def _sync_weight(self) -> None:
        ...
        for o, n in zip(self.diffusion_model_old.parameters(), self.diffusion_model.parameters()):
            o.data.copy_(o.data * (1.0 - self._tau) + n.data * self._tau)
        for o, n in zip(self.bc_diffusion_model_old.parameters(), self.bc_diffusion_model.parameters()):
            o.data.copy_(o.data * (1.0 - self._tau) + n.data * self._tau)
    ...
    def learn(self, batch: Dict) -> Dict[str, float]:
        ...
        #         (self.bc_diffusion_model, self.bc_diffusion_model_old, self.bc_diffusion_model_optim, next_actions),
        #     ]:
        with torch.no_grad():
        ...
        with torch.no_grad():
            next_diff_obss = self.predict(
                    model=self.bc_diffusion_model_old,
                    noise=obss_noise, obs=next_obss, a=next_actions
            ).detach()
            noised_obss = self.add_noise(next_obss, obss_noise, diff_steps) 
            next_noised_obss = self.add_noise(next_diff_obss, obss_noise, diff_steps)            
            next_target = self.bc_diffusion_model_old(
                x=next_noised_obss, obs=next_obss, 
                step=map_i(diff_steps),
                actions=next_actions).detach()

        current_prediction = self.bc_diffusion_model(
            x=noised_obss, obs=obss, 
            step=map_i(diff_steps),
            actions=actions)
        next_prediction = self.bc_diffusion_model(
            x=next_noised_obss, obs=obss, 
            step=map_i(diff_steps),
            actions=actions)
        ...
        self.bc_diffusion_model_optim.zero_grad()
        diffusion_loss.backward()
        self.bc_diffusion_model_optim.step()
        # # for _ in range(n//3):
        # for (model, model_old, model_opt, next_actions_tensor) in [
        #         (self.diffusion_model, self.diffusion_model_old, self.diffusion_model_optim, pred_next_actions),
        #         (self.bc_diffusion_model, self.bc_diffusion_model_old, self.bc_diffusion_model_optim, next_actions),
        ...
        #         next_diff_obss = self.predict(
        #                 model=model_old,
        #                 noise=obss_noise, obs=next_obss, a=next_actions_tensor
        #         ).detach()
        #         noised_obss = self.add_noise(next_obss, obss_noise, diff_steps) 
        #         next_noised_obss = self.add_noise(next_diff_obss, obss_noise, diff_steps)            
        #         next_target = model(
        #             x=next_noised_obss, obs=next_obss, 
        #             step=map_i(diff_steps),
        #             actions=next_actions_tensor).detach()
        ...
        if self._cnt % self._freq == 0:
            ...
            for _ in range(1):
                ...
                obss_noise = torch.randn(obss.shape, device=obss.device)
                with torch.no_grad():
                    bc_pred_obss = self.predict(
                            model=self.bc_diffusion_model_old,
                            noise=obss_noise, obs=next_obss, a=actions
                    ).detach()
                    noised_bc_pred_obss  = self.add_noise(bc_pred_obss, obss_noise, diff_steps)
                    bc_obs_prediction = self.bc_diffusion_model_old(
                        x=noised_bc_pred_obss, obs=obss, 
                        step=map_i(diff_steps),
                        actions=actions
                    )
                obs_prediction = self.diffusion_model(
                    x=noised_bc_pred_obss, obs=obss, 
                    step=map_i(diff_steps),
                    actions=tanh_a_new
                )
                rkl_list.append(((bc_obs_prediction - obs_prediction)**2).sum(-1))
