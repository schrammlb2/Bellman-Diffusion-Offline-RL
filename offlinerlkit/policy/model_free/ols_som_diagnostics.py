...
class SOMDiagnosticPolicy(TD3Policy):
    """
    ...
    def __init__(
        ...
        self.action_reg_weight = 0.1
        self.state_reg_weight = 10
    ...
    def learn(self, batch: Dict) -> Dict[str, float]:
        ...
        def estimate_rkl_divergence():
            ...
            return ((shuffled_obs_prediction - shuffled_obss)**2).sum(-1)


        def estimate_rkl_grad():
            dist = self.actor(obss)
            atanh_a = dist.rsample()[1]
            tanh_a = torch.tanh(atanh_a)
            ...
            atanh_a.retain_grad()
            loss = ((shuffled_obs_prediction - shuffled_obss)**2).mean()

            self.actor_optim.zero_grad()
            loss.backward()
            grad = torch.clone(atanh_a.grad)
            self.actor_optim.zero_grad()
            return grad


        def estimate_rkl_weight_grad():
            dist = self.actor(obss)
            atanh_a = dist.rsample()[1]
            tanh_a = torch.tanh(atanh_a)
        ...
        def estimate_rkl_variance():
            rkl_list = []
            for _ in range(self.num_diffusion_iters):
                rkl_list.append(estimate_rkl_divergence())
            rkl_tensor = torch.stack(rkl_list, -1)

            return {
                "rkl_s": torch.std(rkl_tensor, dim=-1),
                "rkl_m": torch.mean(rkl_tensor, dim=-1),
                "rkl_s_m": torch.std(rkl_tensor, dim=-1).mean(),
                "rkl_s_s": torch.std(rkl_tensor, dim=-1).std(),
                "rkl_m_m": torch.mean(rkl_tensor),
                "rkl_m_s": torch.mean(rkl_tensor, dim=-1).std(),
            }

        def estimate_rkl_grad_variance():
            rkl_list_1 = []
            rkl_list_2 = []
            for _ in range(self.num_diffusion_iters):
                rkl_list_1.append(estimate_rkl_grad())
                rkl_list_2.append(estimate_rkl_grad())

            rkl_tensor_1 = torch.stack(rkl_list_1, -1)
            rkl_tensor_2 = torch.stack(rkl_list_2, -1)

            norm_1 = ((rkl_tensor_1*rkl_tensor_1).mean(-1).sum(-1))**(1/2)
            norm_2 = ((rkl_tensor_2*rkl_tensor_2).mean(-1).sum(-1))**(1/2)

            cos_sim = (rkl_tensor_1*rkl_tensor_2).mean(-1).sum(-1)/(norm_1*norm_2)

            cos_distance_of_means = ((rkl_tensor_1.mean(-1))*(rkl_tensor_2.mean(-1))).sum(-1)/(norm_1*norm_2)

            cos = torch.nn.functional.cosine_similarity
            cos_sim_torch = cos(rkl_tensor_1, rkl_tensor_2, dim=1).mean()
            cos_sim_means_torch = cos(rkl_tensor_1.mean(-1), rkl_tensor_2.mean(-1), dim=1).mean()


            return {
                "metric/a_cos": cos_sim_torch.item(), 
                "metric/a_mean_cos": cos_sim_means_torch.item()
            }


        def estimate_rkl_weight_grad_variance():
            rkl_list_1 = []
            rkl_list_2 = []
            for _ in range(self.num_diffusion_iters):
                rkl_list_1.append(estimate_rkl_weight_grad())
                rkl_list_2.append(estimate_rkl_weight_grad())
            ...
            cos = torch.nn.functional.cosine_similarity
            cos_sim_torch = cos(rkl_tensor_1, rkl_tensor_2, dim=1).mean()
            cos_sim_means_torch = cos(rkl_tensor_1.mean(-1), rkl_tensor_2.mean(-1), dim=-1)

            return {
                "metric/w_cos": cos_sim_torch.item(), 
                "metric/w_m_cos": cos_sim_means_torch.item()
            }

        if self._cnt%20==0: 
            rkl_grad_variance = estimate_rkl_grad_variance()
            rkl_weight_grad_variance = estimate_rkl_weight_grad_variance()
            self._last_rkl_grad_variance = rkl_grad_variance
            self._last_rkl_weight_grad_variance = rkl_weight_grad_variance
        ...
        return_dict.update(self._last_rkl_grad_variance)
        return_dict.update(self._last_rkl_weight_grad_variance)
        return return_dict