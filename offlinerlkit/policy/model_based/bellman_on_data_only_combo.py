...
class RealBellmanCOMBOPolicy(CQLPolicy):
    """
    ...
    def learn(self, batch: Dict) -> Dict[str, float]:
        ...
        if self._max_q_backup:
            with torch.no_grad():
                tmp_next_obss = real_next_obss.unsqueeze(1) \
            ...
            with torch.no_grad():
                next_actions, next_log_probs = self.actforward(real_next_obss)
                next_q = torch.min(
                    self.critic1_old(real_next_obss, next_actions),
                    self.critic2_old(real_next_obss, next_actions)
                )
                if not self._deterministic_backup:
                    next_q -= self._alpha * next_log_probs

        target_q = real_rewards + self._gamma * (1 - real_terminals) * next_q
