...
class DiffusionCOMBOPolicy(COMBOPolicy):
    """
    ...
    def learn(self, batch: Dict) -> Dict[str, float]:
        ...
        pred_obss, diffusion_loss = self.diffusion_model.update_diffusion(real_batch, lambda x: self.actforward(x)[0])
        result = super().learn(batch)
        result["loss/diffusion"] = diffusion_loss.item()
        return result
