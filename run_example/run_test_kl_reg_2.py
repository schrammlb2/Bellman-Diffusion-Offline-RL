...
from offlinerlkit.policy import TestKLRegPolicy
...
def get_args():
    ...
    parser.add_argument("--algo-name", type=str, default="modified_test_kl_reg")
...
def train(args=get_args()):
    ...
    diffusion_hidden_dims = [256, 256, 256, 256]
    # backbone = MLP
    ...
    critic2_backbone = NormedMLP(input_dim=np.prod(args.obs_shape)+args.action_dim, 
        hidden_dims=single_eval_hidden_dims)
    diffusion_backbone = MPDenseNet(input_dim=2*np.prod(args.obs_shape)+args.action_dim + 1, 
        hidden_dims=diffusion_hidden_dims)
    data_diffusion_backbone = MPDenseNet(input_dim=np.prod(args.obs_shape) + 1, 
        hidden_dims=diffusion_hidden_dims)
