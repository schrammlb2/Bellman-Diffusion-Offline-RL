...
def train(args=get_args()):
    ...
    bc_diffusion_backbone = NormedMLP(input_dim=2*np.prod(args.obs_shape)+args.action_dim + 1, 
        hidden_dims=diffusion_hidden_dims)
    ...
    bc_diffusion_model = DiffusionModel(bc_diffusion_backbone, obs_dim=np.prod(args.obs_shape), device=args.device)
    actor_optim = torch.optim.Adam(actor.parameters(), lr=args.actor_lr)
    critic1_optim = torch.optim.Adam(critic1.parameters(), lr=args.critic_lr)
    critic2_optim = torch.optim.Adam(critic2.parameters(), lr=args.critic_lr)
    diffusion_optim = torch.optim.Adam(diffusion_model.parameters(), lr=args.critic_lr)
    bc_diffusion_optim = torch.optim.Adam(bc_diffusion_model.parameters(), lr=args.critic_lr)
