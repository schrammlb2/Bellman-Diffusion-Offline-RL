...
def get_args():
    ...
    parser.add_argument("--algo-name", type=str, default="rebrac_som")
    ...
    parser.add_argument("--relative-state-reg-weight", type=float, default=0.1)
...
def train(args=get_args()):
    ...
    data_diffusion_backbone = NormedMLP(input_dim=np.prod(args.obs_shape) + 1, 
        hidden_dims=diffusion_hidden_dims)
    # diffusion_backbone = DenseNet(input_dim=2*np.prod(args.obs_shape)+args.action_dim + 1, 
    #     hidden_dims=diffusion_hidden_dims)
    # data_diffusion_backbone = DenseNet(input_dim=np.prod(args.obs_shape) + 1, 
    #     hidden_dims=[256,256])

    critic1 = Critic(critic1_backbone, args.device)
    critic2 = Critic(critic2_backbone, args.device)
    diffusion_model = DiffusionNetwork(
        diffusion_backbone, output_dim=np.prod(args.obs_shape), device=args.device)
    data_diffusion_model = UnconditionalDiffusionNetwork(
        data_diffusion_backbone, output_dim=np.prod(args.obs_shape), device=args.device)
    ...
    policy = ReBRACSOMPolicy(
        ...
        relative_state_reg_weight=args.relative_state_reg_weight,
        scaler=scaler,
