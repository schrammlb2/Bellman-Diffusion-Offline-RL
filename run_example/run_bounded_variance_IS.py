...
from offlinerlkit.policy import BoundedVarianceISPolicy
...
def train(args=get_args()):
    ...
    # create policy model
    value_actor_backbone = MLP(input_dim=np.prod(args.obs_shape), hidden_dims=args.hidden_dims, dropout_rate=args.dropout_rate)
    bc_actor_backbone = MLP(input_dim=np.prod(args.obs_shape), hidden_dims=args.hidden_dims, dropout_rate=args.dropout_rate)
    ...
    bc_dist = TanhDiagGaussian(
        latent_dim=getattr(bc_actor_backbone, "output_dim"),
        output_dim=args.action_dim,
        unbounded=True,
        conditioned_sigma=True,
        max_mu=args.max_action
    )
    value_dist = TanhDiagGaussian(
        latent_dim=getattr(value_actor_backbone, "output_dim"),
        output_dim=args.action_dim,
        unbounded=True,
        conditioned_sigma=True,
        max_mu=args.max_action
    )
    bc_actor = ActorProb(bc_actor_backbone, bc_dist, args.device)
    value_actor = ActorProb(value_actor_backbone, value_dist, args.device)
    ...
    bc_actor_optim = torch.optim.Adam(bc_actor.parameters(), lr=args.actor_lr)
    value_actor_optim = torch.optim.Adam(value_actor.parameters(), lr=args.actor_lr)
    ...
    # create IQL policy
    policy = BoundedVarianceISPolicyPolicy(
        bc_actor,
        value_actor,
        critic_q1,
        critic_q2,
