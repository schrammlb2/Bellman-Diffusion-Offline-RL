...
from offlinerlkit.nets import MLP, NormedMLP, MPDenseNet
...
from offlinerlkit.policy import SACKLPolicy
...
def get_args():
    ...
    parser.add_argument("--algo-name", type=str, default="sackl")
    ...
    parser.add_argument("--action-reg-weight", type=float, default=0.01)
...
def train(args=get_args()):
    ...
    # densenet_hidden = [256, 256]
    # critic1_backbone = MPDenseNet(input_dim=np.prod(args.obs_shape) + args.action_dim, hidden_dims=densenet_hidden)
    # critic2_backbone = MPDenseNet(input_dim=np.prod(args.obs_shape) + args.action_dim, hidden_dims=densenet_hidden)
    ...
    if args.auto_alpha:
        ...
        log_alpha = torch.tensor(0.01, requires_grad=True, device=args.device)
