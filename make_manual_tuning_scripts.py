...
# no_q=False
seed=1
for no_q in [True]:#, False]:
        q_string = "no_q" if no_q else "q"
        template = "sbatch_template.sh"
        alg_list = [f"sequential_rebrac_som_{q_string}"]
for alg in alg_list:
	base_dir = f"{pwd}/tuning/manual_{alg}_scripts"        
	# script_dir = "/common/home/lbs105/Desktop/Bellman-Diffusion-Offline-RL/pyrallis_scripts"
	script_dir = f"{pwd}/pyrallis_scripts"
	method = f"{script_dir}/manual_seq_som_grid_search.py --config={script_dir}/configs/offline/rebrac/"
	os.mkdir(base_dir)
		...
		for env in envs:
			...
			for _ in range(1):
				...
				with open(loc, "a") as file:
					...
					file.write(f"python {method}{env}/{dataset}_v2.yaml --train_seed={seed} --no_q={no_q}")
