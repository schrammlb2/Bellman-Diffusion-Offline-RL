...
def train_meta(config: Config):
    ...
    ray.init(_temp_dir="/common/home/lbs105/ray/", num_cpus=1, num_gpus=1)

    ray_config = vars(config)
    ray_config["relative_state_bc_coef"]= tune.qloguniform(1, 100, 1)
    ray_config["div"]= tune.qloguniform(1, 100, 1)    
    algo = OptunaSearch()

    num_samples = 30
    trainable_with_resources = tune.with_resources(train, {"gpu": 0.25})
