...
def train(config):
    ...
    # config.relative_state_bc_coef = 100
    config.actor_learning_rate /= config.div
    diffusion_learning_rate = config.critic_learning_rate/config.div
    betas = (.9, .999) #Default
    ...
    r_result = {"reward": result['last_10_performance']}
    # tune.report(**result)
...
@pyrallis.wrap()
def train_meta(config: Config):
    from dataclasses import fields

    q = "no_q" if config.no_q else "q"
    ray_config = vars(config)
    with open(f"manual_tuning_results/{q}_s{config.train_seed}/{config.dataset_name}", "w") as file:
        pass
    for state_bc in [1,10,100]:
        for div in [1, 10, 100]:
            ray_config = vars(config)
            ray_config["relative_state_bc_coef"]= state_bc
            ray_config["div"]= div
            result = train(ray_config)
            settings = {
                "dataset_name": ray_config["dataset_name"],
                "seed": ray_config["train_seed"],
                "relative_state_bc_coef": state_bc,
                "div": div,
                "result": result,
            }
            with open(f"manual_tuning_results/{q}_s{config.train_seed}/{config.dataset_name}", "a") as file:
                file.write(format_dict(settings) + "\n\n")
