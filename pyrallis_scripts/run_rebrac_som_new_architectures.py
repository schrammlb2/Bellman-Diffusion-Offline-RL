...
from offlinerlkit.nets import MLP, NormedMLP, DenseNet, MPDenseNet, ExtremelyNormalMLP, ExtremelyNormalResnet
...
from offlinerlkit.policy import ReBRACSOMPolicy
...
def train(config: Config):
    ...
    # config.actor_learning_rate *= 1e-2
    ...
    actor_backbone = ExtremelyNormalMLP(input_dim=np.prod(obs_shape), hidden_dims=hidden_dims)
    critic1_backbone = ExtremelyNormalMLP(input_dim=np.prod(obs_shape)+action_dim, hidden_dims=hidden_dims)
    critic2_backbone = ExtremelyNormalMLP(input_dim=np.prod(obs_shape)+action_dim, hidden_dims=hidden_dims)
    actor = Actor(actor_backbone, action_dim, max_action=max_action, device=device)
    diffusion_backbone = ExtremelyNormalMLP(input_dim=2*np.prod(obs_shape)+action_dim + 1, 
        hidden_dims=diffusion_hidden_dims)
    data_diffusion_backbone = ExtremelyNormalMLP(input_dim=np.prod(obs_shape) + 1, 
        hidden_dims=diffusion_hidden_dims)
