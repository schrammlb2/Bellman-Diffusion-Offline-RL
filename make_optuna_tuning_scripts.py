...
for alg in alg_list:
	base_dir = f"{pwd}/tuning/optuna_{alg}_scripts"        
	# script_dir = "/common/home/lbs105/Desktop/Bellman-Diffusion-Offline-RL/pyrallis_scripts"
	script_dir = f"{pwd}/pyrallis_scripts"
	method = f"{script_dir}/tune_seq_som.py --config={script_dir}/configs/offline/rebrac/"
	os.mkdir(base_dir)
	...
	for dataset in datasets:
		...
		for env in envs:
			...
			for seed in range(1):
				...
				with open(run_env_loc, "a") as file:
					file.write("\n")
					file.write(f"sbatch -G 2 {filename}\n")
					file.write(f"sleep 30s\n")
